...
const Sidebar = ({ activeTab, onTabChange }) => {
  ...
  return (
    <aside className="bg-background/80 backdrop-blur-2xl w-64 h-full flex flex-col py-8 z-40 hidden md:flex font-body text-sm relative border-r border-white/5">
      <div className="px-8 mb-12 flex items-center gap-4 relative">
        <div className="w-10 h-10 bg-primary/20 text-primary rounded-xl flex items-center justify-center shadow-2xl shadow-primary/10 border border-primary/20">
          <span className="material-symbols-outlined text-xl">account_balance_wallet</span>
        </div>
        <div>
          <h2 className="text-lg font-black text-white leading-none tracking-tight font-headline uppercase">Vault <span className="text-primary">X</span></h2>
        ...
        {menuItems.map((item) => (
          <div 
            ...
            className={`flex items-center gap-3.5 px-5 py-3 rounded-xl transition-all duration-500 cursor-pointer group mb-1 ${
              activeTab === item.id 
                ? 'bg-primary/10 text-primary border border-primary/20 shadow-[0_0_20px_rgba(45,212,191,0.1)]' 
      ...
      <div className="px-4 mt-auto space-y-1 relative pt-6 border-t border-white/5">
        <div className="flex items-center gap-3.5 text-on-surface-variant/60 px-4 py-3 hover:text-white hover:bg-white/5 rounded-xl transition-all duration-300 cursor-pointer group">
