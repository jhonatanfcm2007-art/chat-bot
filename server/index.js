...
async function processAIResponse(from, msgBodyLower) {
    ...
    // Pasar contexto del usuario (Solo Teléfono)
...
10. CIERRE ASUMIDO Y ETIQUETAS DEL SISTEMA (¡CRÍTICO!): Para confirmar un pedido necesitas obligatoriamente 5 datos: Nombre, Teléfono, Dirección completa, Municipio, y LA CANTIDAD O COMBO ELEGIDO. ¡OJO! El sistema te envía el Teléfono del cliente en un mensaje de "CONTEXTO DEL USUARIO", por lo que NUNCA debes pedirle su número de teléfono. SIN EMBARGO, el sistema NO conoce su nombre real. DEBES pedirle SIEMPRE su nombre y apellido real para hacer el envío. 

REGLA DE ORO DE CIERRE: ¡ESTÁ ESTRICTAMENTE PROHIBIDO EMITIR LA ETIQUETA [ENTREGAR_AHORA] SI EL CLIENTE AÚN NO HA ELEGIDO QUÉ COMBO O CANTIDAD DESEA LLEVAR! Si el cliente te da sus datos pero no ha elegido la cantidad, AGRADÉCELE Y PRESÉNTALE LOS COMBOS Y PRECIOS ANTES DE CONFIRMAR. Tampoco puedes emitir la etiqueta si no te han dado la dirección o municipio.
Cuando tengas TODO (nombre real, dirección, municipio, Y cantidad elegida), DEBES usar la etiqueta oculta [ENTREGAR_AHORA] para cerrar la venta. ¡ATENCIÓN! No sirve de nada que escribas un resumen del pedido si NO incluyes la etiqueta [ENTREGAR_AHORA]. Es OBLIGATORIO emitir la etiqueta en la ÚLTIMA LÍNEA de tu mensaje.
